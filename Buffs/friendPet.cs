...
namespace Heylookamod.Buffs
{
    public class friendPet : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            player.GetModPlayer<HeylookamodPlayer>(mod).friendPet = true;
            ...
            petProjectileNotSpawned = player.ownedProjectileCounts[mod.ProjectileType("ClownCopter")] <= 0;
            ...
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                ...
                Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, mod.ProjectileType("ClownCopter"), 0, 0f, player.whoAmI, 0f, 0f);
